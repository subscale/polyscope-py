...
from polyscope.core import (
    str_to_datatype,
    str_to_vectortype,
    str_to_param_coords_type,
    str_to_param_viz_style,
    str_to_back_face_policy,
...
class SurfaceMesh:
    ...
    def __init__(self, name=None, vertices=None, faces=None, instance=None):
        ...
        if instance is not None:
            ...
            if isinstance(faces, np.ndarray):
                ...
                if vertices.shape[1] == 3:
                    self.bound_mesh = psb.register_surface_mesh_list(
                        name, vertices, faces
                    )
                elif vertices.shape[1] == 2:
                    self.bound_mesh = psb.register_surface_mesh_list2D(
                        name, vertices, faces
                    )
    ...
    def set_vertex_permutation(self, perm, expected_size=None):
        if len(perm.shape) != 1 or perm.shape[0] != self.n_vertices():
            raise ValueError("'perm' should be an array with one entry per vertex")
        if expected_size is None:
            expected_size = 0
        self.bound_mesh.set_vertex_permutation(perm, expected_size)

    def set_face_permutation(self, perm, expected_size=None):
        if len(perm.shape) != 1 or perm.shape[0] != self.n_faces():
            raise ValueError("'perm' should be an array with one entry per face")
        if expected_size is None:
            expected_size = 0
    ...
    def set_all_permutations(
        self,
        vertex_perm=None,
        vertex_perm_size=None,
        face_perm=None,
        face_perm_size=None,
        edge_perm=None,
        edge_perm_size=None,
        corner_perm=None,
        corner_perm_size=None,
        halfedge_perm=None,
        halfedge_perm_size=None,
    ):

        if vertex_perm is not None:
            self.set_vertex_permutation(vertex_perm, vertex_perm_size)
        if face_perm is not None:
            self.set_face_permutation(face_perm, face_perm_size)
    ...
    def set_vertex_tangent_basisX(self, vectors):
        if (
            len(vectors.shape) != 2
            or vectors.shape[0] != self.n_vertices()
            or vectors.shape[1] not in (2, 3)
        ):
    ...
    def set_face_tangent_basisX(self, vectors):
        if (
            len(vectors.shape) != 2
            or vectors.shape[0] != self.n_faces()
            or vectors.shape[1] not in (2, 3)
        ):
    ...
    def add_parameterization_quantity(
        ...
        if defined_on == "vertices":
            if values.shape[0] != self.n_vertices():
                raise ValueError("'values' should be a length n_vertices array")
            q = self.bound_mesh.add_vertex_parameterization_quantity(
                name, values, coords_type_enum
            )
        elif defined_on == "corners":
            if values.shape[0] != self.n_corners():
                raise ValueError("'values' should be a length n_faces array")
            q = self.bound_mesh.add_corner_parameterization_quantity(
                name, values, coords_type_enum
            )
        else:
            raise ValueError(
    ...
    def add_vector_quantity(
        ...
        if defined_on == "vertices":
            ...
            if values.shape[1] == 2:
                q = self.bound_mesh.add_vertex_vector_quantity2D(
                    name, values, str_to_vectortype(vectortype)
                )
            elif values.shape[1] == 3:
                q = self.bound_mesh.add_vertex_vector_quantity(
                    name, values, str_to_vectortype(vectortype)
                )
            ...
            if values.shape[1] == 2:
                q = self.bound_mesh.add_face_vector_quantity2D(
                    name, values, str_to_vectortype(vectortype)
                )
            elif values.shape[1] == 3:
                q = self.bound_mesh.add_face_vector_quantity(
                    name, values, str_to_vectortype(vectortype)
                )
    ...
    def add_intrinsic_vector_quantity(
        self,
        name,
        values,
        n_sym=1,
        ...
        if defined_on == "vertices":
            ...
            q = self.bound_mesh.add_vertex_intrinsic_vector_quantity(
                name, values, n_sym, str_to_vectortype(vectortype)
            )
            ...
            q = self.bound_mesh.add_face_intrinsic_vector_quantity(
                name, values, n_sym, str_to_vectortype(vectortype)
            )
    ...
    def add_one_form_vector_quantity(
        ...
        q = self.bound_mesh.add_one_form_intrinsic_vector_quantity(
