...
class CurveNetwork:
    ...
    def __init__(self, name=None, nodes=None, edges=None, instance=None):
        ...
        if instance is not None:
            ...
            if isinstance(edges, str):
                ...
                if edges not in ["line", "loop"]:
                    raise ValueError(
                        "bad edges setting, if not numpy array must be in 'line' or 'loop', got "
                        + str(edges)
                    )

                if nodes.shape[1] == 3:
                    if edges == "line":
                        self.bound_network = psb.register_curve_network_line(
                            name, nodes
                        )
                    elif edges == "loop":
                        self.bound_network = psb.register_curve_network_loop(
                            name, nodes
                        )
                elif nodes.shape[1] == 2:
                    if edges == "line":
                        self.bound_network = psb.register_curve_network_line2D(
                            name, nodes
                        )
                    elif edges == "loop":
                        self.bound_network = psb.register_curve_network_loop2D(
                            name, nodes
                        )
            else:
                ...
                if (len(edges.shape) != 2) or (edges.shape[1] != 2):
                    raise ValueError(
                        "curve network edges should have shape (N_edge,2); shape is "
                        + str(edges.shape)
                    )
                ...
                if nodes.shape[1] == 3:
                    ...
                    self.bound_network = psb.register_curve_network2D(
                        name, nodes, edges
                    )
    ...
    def check_shape(self, points):
        ...
        if (len(points.shape) != 2) or (points.shape[1] not in (2, 3)):
            raise ValueError(
                "curve network node positions should have shape (N,3); shape is "
                + str(points.shape)
            )
    ...
    def add_scalar_quantity(
        ...
        if defined_on == "nodes":
            ...
            q = self.bound_network.add_node_scalar_quantity(
            ...
            q = self.bound_network.add_edge_scalar_quantity(
                name, values, str_to_datatype(datatype)
            )
        else:
            raise ValueError(
                "bad `defined_on` value {}, should be one of ['nodes', 'edges']".format(
                    defined_on
                )
            )
    ...
    def add_vector_quantity(
        ...
        if defined_on == "nodes":
            ...
            if values.shape[1] == 2:
                q = self.bound_network.add_node_vector_quantity2D(
                    name, values, str_to_vectortype(vectortype)
                )
            elif values.shape[1] == 3:
                q = self.bound_network.add_node_vector_quantity(
                    name, values, str_to_vectortype(vectortype)
                )
            ...
            if values.shape[1] == 2:
                q = self.bound_network.add_edge_vector_quantity2D(
                    name, values, str_to_vectortype(vectortype)
                )
            elif values.shape[1] == 3:
                q = self.bound_network.add_edge_vector_quantity(
                    name, values, str_to_vectortype(vectortype)
                )
