...
class PointCloud:
    ...
    def add_scalar_quantity(
        self, name, values, enabled=None, datatype="standard", vminmax=None, cmap=None
    ):
        if len(values.shape) != 1 or values.shape[0] != self.n_points():
            raise ValueError("'values' should be a length-N array")

        q = self.bound_cloud.add_scalar_quantity(
    ...
    def add_vector_quantity(
        ...
        if values.shape[1] == 2:
            q = self.bound_cloud.add_vector_quantity2D(
                name, values, str_to_vectortype(vectortype)
            )
        elif values.shape[1] == 3:
            q = self.bound_cloud.add_vector_quantity(
                name, values, str_to_vectortype(vectortype)
            )
