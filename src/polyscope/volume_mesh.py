...
class VolumeMesh:
    ...
    def __init__(
        ...
        if instance is not None:
            ...
            if mixed_cells is not None and (tets is not None or hexes is not None):
                raise ValueError(
                    "specify EITHER mixed_cells OR tets/hexes but not both"
                )
            ...
            if mixed_cells is not None:
                ...
                if tets is None:
                    ...
                    self.bound_mesh = psb.register_tet_hex_mesh(
                        name, vertices, tets, hexes
                    )
    ...
    def check_shape(self, points):
        ...
        if len(points.shape) != 2 or points.shape[1] != 3:
            raise ValueError(
                "volume mesh vertex positions should have shape (N,3); shape is "
                + str(points.shape)
            )
    ...
    def check_index_array(self, arr, dim, name):
        ...
        if not isinstance(arr, np.ndarray):
            raise ValueError(
                "volume mesh {} array should be a numpy array, but is {}".format(
                    name, type(arr)
                )
            )
        if not issubclass(arr.dtype.type, np.integer):
            raise ValueError(
                "volume mesh {} array should be an integer array".format(name)
            )

        # check the dimension
        if (len(arr.shape) != 2) or (arr.shape[1] != dim):
            raise ValueError(
                "volume mesh {} array should have shape (N,{}); shape is {}".format(
                    name, dim, str(arr.shape)
                )
            )
    ...
    def add_scalar_quantity(
        ...
        if defined_on == "vertices":
            ...
            q = self.bound_mesh.add_cell_scalar_quantity(
                name, values, str_to_datatype(datatype)
            )
        else:
            raise ValueError(
                "bad `defined_on` value {}, should be one of ['vertices', 'cells']".format(
                    defined_on
                )
            )
    ...
    def add_vector_quantity(
        ...
        if defined_on == "vertices":
            ...
            if values.shape[0] != self.n_cells():
                raise ValueError("'values' should be a length n_cells array")
            q = self.bound_mesh.add_cell_vector_quantity(
                name, values, str_to_vectortype(vectortype)
            )
        else:
            raise ValueError(
...
def register_volume_mesh(
    ...
    p = VolumeMesh(
        name, vertices=vertices, tets=tets, hexes=hexes, mixed_cells=mixed_cells
    )
